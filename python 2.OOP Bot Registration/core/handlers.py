...
from core.database import DatabaseTodo
...
class BotHandlers:
    ...
    async def start_command(self, message: types.Message):
        await message.answer("Привет! Викторина готова.\nНапиши /quiz для начала или /register для регистрации!")
    ...
    async def send_question(self, chat_id, user_id):
        ...
        keyboard = InlineKeyboardMarkup(
            inline_keyboard=[[InlineKeyboardButton(text=opt, callback_data=opt)] for opt in question_data["option"]]
        )

        caption_text = f"Вопрос {data['q_index'] + 1}/15: {question_data['question']}"
        photo = FSInputFile(question_data["image"])
        await self.bot.send_photo(chat_id, photo=photo, caption=caption_text, reply_markup=keyboard)
    ...
    async def end_quiz(self, chat_id, user_id):
        ...
        await self.bot.send_message(chat_id, f"Тест пройден!\nПравильно: {res['score']}\nОшибок: {res['wrong']}")
